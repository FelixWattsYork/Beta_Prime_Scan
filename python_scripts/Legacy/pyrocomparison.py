...
class PyroComparison:
    ...
    def _prepare_runs(self) -> List[Dict[str, Any]]:
        ...
        for code, variations in self.codes.items():
            if not variations:
                ...
                for var in variations:
                    ...
                    funcs_to_add_keys =  var.get("funcs", {})
                    print(funcs_to_add_keys)
                    for funcs_to_add_keys in funcs_to_add_keys:
                        pyro_scan.add_parameter_func(funcs_to_add_keys[0], funcs_to_add_keys[1], funcs_to_add_keys[2])
        ...
        for run in runs:
            ...
            print(run["pyroscan"])
            print(FILE_NAME_DICT[code])
            print(self.output_loc / f"{run["name"]}_Runs")
            run["pyroscan"].write(
