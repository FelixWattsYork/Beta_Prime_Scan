...
def plot_2d(runs,names,plot_location, Gaussian=False,parameter_1_range = None,parameter_2_range = None):
        ...
        for i in range (0,2):
            ...
            if i == 0:
                if parameter_1_range != None:
                    list_to_loop_over = growth_rate_list[0].coords[major_cord][slice(*parameter_1_range)]
            elif i == 1:
                if parameter_2_range != None:
                    list_to_loop_over = growth_rate_list[0].coords[major_cord][slice(*parameter_2_range)]
            ...
            for j, coor in enumerate(list_to_loop_over):
                ...
                if Gaussian:
                    ...
                    y1_max = Gaussian_growth_rate.sel({major_cord: coor}).sel(output="max_value")
                    y1_min = Gaussian_growth_rate.sel({major_cord: coor}).sel(output="min_value")

                    x2 = Gaussian_mode_frequency[minor_cord]
                    y2 = Gaussian_mode_frequency.sel({major_cord: coor}).sel(output="value")
                    y2_max = Gaussian_mode_frequency.sel({major_cord: coor}).sel(output="max_value")
                    y2_min = Gaussian_mode_frequency.sel({major_cord: coor}).sel(output="min_value")

                    # Plot with shaded error region and dotted central line
                    # --- Growth rate ---
                    ax1.plot(x1, y1, linestyle=':', color='green', label='GS2_GP')  # dotted central line
                    ax1.fill_between(x1, y1_min, y1_max, color='green', alpha=0.2)       # light shading

                    # --- Mode frequency ---
                    ax2.plot(x2, y2, linestyle=':', color='green', label='GS2_GP')  # dotted central line
